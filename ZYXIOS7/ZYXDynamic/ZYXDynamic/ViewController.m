...
- (IBAction)snap:(id)sender
{
    ...
    //迅速移动行为创建后，只有damping可以修改。如果想再次移动到新点，需要移除上一个行为
    //此属性值越大，速度越慢
    snap.damping = 0.25;
...
{
    //附着行为必须有一个视图动了，另一个才会动
    UIAttachmentBehavior *attach1 = [[UIAttachmentBehavior alloc] initWithItem:self.box1 offsetFromCenter:UIOffsetMake(25, 25) attachedToAnchor:self.box1.center];
    ...
    UIPushBehavior *push = [[UIPushBehavior alloc] initWithItems:@[self.box2] mode:UIPushBehaviorModeInstantaneous];//instantaneous瞬间发生，力不会再增加
...
{
    UIPushBehavior *push = [[UIPushBehavior alloc] initWithItems:@[self.box1] mode:UIPushBehaviorModeContinuous];  //持续加力，也就是速度会越来越快
...
{
    UICollisionBehavior *collison = [[UICollisionBehavior alloc] initWithItems:@[self.box1, self.box2]];
    [self.dynamicAnimator addBehavior:collison];
...
- (void)addTemporaryBehavior:(UIDynamicBehavior *)behavior
{
    ...
    [self.dynamicAnimator performSelector:@selector(removeBehavior:) withObject:behavior afterDelay:1.0f];
}
...
- (CGPoint)randomPoint
{
    ...
    //arc4random_uniform返回一个不大于参数的随机数
